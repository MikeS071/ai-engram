...
class LinkedInClient:
    def __init__(self, vault: TokenVault | None = None) -> None:
        ...
        self.publish_url = os.getenv("LINKEDIN_PUBLISH_URL")
        self.verify_url = os.getenv("LINKEDIN_VERIFY_URL")
        self.vault = vault
    ...
    def publish_article(
        ...
        if not self.publish_url:
            raise RuntimeError("LINKEDIN_PUBLISH_URL not configured")
        ...
        if resp.status_code >= 400:
            raise RuntimeError(f"LinkedIn publish failed: status={resp.status_code}")
        data = self._safe_json(resp)
        external_id = str(data.get("external_post_id") or data.get("id") or "").strip()
        if not external_id:
            raise RuntimeError("LinkedIn publish response missing external post id")
        return external_id
    ...
    def _safe_json(self, resp: httpx.Response) -> dict[str, Any]:
        try:
            ...
            raise RuntimeError("LinkedIn API returned non-JSON response") from exc
